...
def get_job_time(workflow_run_id, token=None):
    ...
    result = requests.get(url, headers=headers) if requests.get(url, headers=headers).status_code == 200 else {'jobs': []}
    job_time = {}
    ...
    try:
        ...
        for i in range(pages_to_iterate_over):
            result = requests.get(url + f"&page={i + 2}", headers=headers) if requests.get(url + f"&page={i + 2}", headers=headers).status_code == 200 else {'jobs': []}
            job_time.update({job["name"]: extract_time_from_single_job(job) for job in result["jobs"]})
