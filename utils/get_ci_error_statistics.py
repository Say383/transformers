...
def get_job_links(workflow_run_id, token=None):
    ...
    url = f"https://api.github.com/repos/huggingface/transformers/actions/runs/{workflow_run_id}/jobs?per_page=100&token={token}"
    ...
    try:
        ...
        for i in range(1, pages_to_iterate_over):
...
def get_artifacts_links(worflow_run_id, token=None):
    ...
    url = f"https://api.github.com/repos/huggingface/transformers/actions/runs/{worflow_run_id}/artifacts?per_page=100&token={token}"
    result = requests.get(url, headers=headers).json()
    artifacts = {}
